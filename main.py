...
MEMBER_DIR_URL = "https://example.com"
...
def get_country_options(driver):
    ...
    countries = [(option.get_attribute("value"), option.text.strip()) for option in select.options if option.get_attribute("value") and option.get_attribute("value") != "[ALL]"]
...
def scrape_name_address_map(driver, country_code, country_name, index, total):
    ...
    try:
        ...
        while True:
            try:
                result_count_div = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div.membership-dir-resultcount")))
                _, end, total_results = parse_result_count(result_count_div.text.strip())
                if end >= total_results:
                    break
                next_btn = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.ID, "dnn_ctr2729_DNNWebControlContainer_ctl00_Next")))
        ...
        for container in containers:
            ...
            address_div = container.select_one("div.DISPLAY_LINE_1")
...
def main():
    ...
    try:
        ...
        for idx, (code, name) in enumerate(countries, 1):
            partial_map = scrape_name_address_map(driver, code, name, idx, len(countries))
            all_map.update(partial_map)
            save_map(all_map)  
        print(f"\n[Done] Total entries saved: {len(all_map)}")
    finally:
        if driver:
            driver.quit()
